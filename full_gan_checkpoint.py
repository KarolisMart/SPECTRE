...
from util.model_helper import zero_diag, rand_rot, rand_rewire, eigval_noise, sort_eigvecs, interpolate_eigvecs, reorder_adj, categorical_permute
...
class SPECTRE(pl.LightningModule):
    ...
    def __init__(
        ...
        gp_include_unpermuted: bool = False, ppgn_data_channels_mult: int = 1.0, skip_noise_preprocess: bool = False, qm9: bool = False
    ) -> None:
        super().__init__()
    ...
    def _disc_step(self, real_eigval: torch.Tensor, real_eigvec: torch.Tensor, mask: torch.Tensor, adj: torch.Tensor, real_edge_features: torch.Tensor, real_node_features: torch.Tensor) -> torch.Tensor:
        ...
        if self.hparams.noisy_disc:
            ...
            if self.hparams.qm9:
                if self.hparams.disc_noise_rewire > 0:
                    noisy_adj, _ = rand_rewire(adj, mask=mask, fraction=self.hparams.disc_noise_rewire, noise=self.hparams.adj_noise, edge_features=real_edge_features)
                else:
                    noisy_adj = torch.clamp(adj + (torch.randn_like(adj) * self.hparams.adj_noise), min=0, max=1)
            else:
                if self.hparams.disc_noise_rewire > 0:
                    noisy_adj = rand_rewire(adj, mask=mask, fraction=self.hparams.disc_noise_rewire, noise=self.hparams.adj_noise)
                else:
                    noisy_adj = torch.clamp(adj + (torch.randn_like(adj) * self.hparams.adj_noise), min=0, max=1)
        else:
        ...
        noisy_adj = reorder_adj(noisy_adj, noisy_real_eigvec_indices)
        ...
        if self.hparams.lambda_only:
            ...
            # Train with real
            real_pred = self.discriminator(noisy_real_eigval, noisy_real_eigvec, mask, adj, node_features=real_node_features, edge_features=real_edge_features)
            ...
            if self.hparams.noisy_disc:
                ...
                if self.hparams.qm9:
                    if self.hparams.disc_noise_rewire > 0:
                        noisy_fake_adj, _ = rand_rewire(fake_adj, mask=mask, fraction=self.hparams.disc_noise_rewire, noise=self.hparams.adj_noise, edge_features=fake_edge_features)
                    else:
                        noisy_fake_adj = torch.clamp(fake_adj + (torch.randn_like(adj) * self.hparams.adj_noise), min=0, max=1)
                else:
                    if self.hparams.disc_noise_rewire > 0:
                        noisy_fake_adj = rand_rewire(fake_adj, mask=mask, fraction=self.hparams.disc_noise_rewire, noise=self.hparams.adj_noise)
                    else:
                        noisy_fake_adj = torch.clamp(fake_adj + (torch.randn_like(adj) * self.hparams.adj_noise), min=0, max=1)
            else:
                noisy_mixed_eigval = mixed_eigval
            ...
            fake_adj = reorder_adj(fake_adj, noisy_mixed_eigvec_indices)
            
            fake_pred = self.discriminator(noisy_mixed_eigvec_eigval, noisy_mixed_eigvec, mask, fake_adj, node_features=fake_node_features, edge_features=fake_edge_features)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("epoch/max_allocated_mem", round(torch.cuda.max_memory_allocated()/1024.0**2))
        self.log("epoch/max_cached_mem", round(torch.cuda.max_memory_reserved()/1024.0**2))
            ...
            if self.hparams.lambda_only:
                ...
                for test_type in test_types:
                    ...
                    for i in range(adj.size(0)):
                        ...
                        fake_eigvecs.append(fake_eigvec[i, :n_nodes[i], :self.hparams.k_eigval].cpu().detach().numpy())
                        if self.hparams.qm9:
                            #  Add node features as edege "weight"
                    ...
                    if batch_idx==0:
                        ...
                        try:
                            ...
                            fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(8,8))
                            ax.plot(real_eigval[0].cpu().detach().numpy())
                            ax.plot(rec_eigval[eigv_offset:self.hparams.k_eigval].cpu().detach().numpy())
                        ...
                        for i in range(1,3):
                            ...
                            try:
                                ...
                                fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(8,8))
                                ax.plot(real_eigval[0].cpu().detach().numpy())
                                ax.plot(rec_eigval_i[:self.hparams.k_eigval].cpu().detach().numpy())
                    ...
                    for i in range(L.size(0)):
                        try:
                            rec_eigval = torch.linalg.eigvalsh(L[i, :n_nodes[i], :n_nodes[i]])
                ...
                if self.hparams.lambda_only:
                    ...
                    for test_type in test_types:
                        ...
                        for measure_type in measure_types:
                            mmd_spectral_filter = spectral_filter_stats(true_eigvecs, true_eigvals, fake_eigvecs, fake_eigvals, compute_emd=(measure_type=='emd'))
                            self.log(f'validation_loss_{test_type}{cond_data_sufix}{model_type}/{measure_type}_spectral_filter', mmd_spectral_filter)
                    ...
                    for test_type in test_types:
                        ...
                        if self.hparams.qm9:
                            ...
                            for measure_type in measure_types:
                                ...
                                mmd_spectral_filter_graph = spectral_filter_stats(true_graph_eigvecs, true_graph_eigvals, fake_graph_eigvecs, fake_graph_eigvals, compute_emd=(measure_type=='emd'))
                                self.log(f'validation_loss_{test_type}{cond_data_sufix}{model_type}/{measure_type}_spectral_filter_graph', mmd_spectral_filter_graph)
                            ...
                            mean_MMD_ratio_5 = ((mmd_degree / (self.trainer.datamodule.train_mmd_degree + 1e-6)) +
                                                ...
                                                (mmd_spectral_filter_graph / (self.trainer.datamodule.train_mmd_spectral_filter_graph+ 1e-6))) / 5
            ...
            if self.hparams.lambda_only:
                ...
                for test_type in test_types:
                    ...
                    for i in range(adj.size(0)):
                        ...
                        true_eigvecs.append(real_eigvec[i, :n_nodes[i], :self.hparams.k_eigval].cpu().detach().numpy())
                        fake_eigvecs.append(fake_eigvec[i, :n_nodes[i], :self.hparams.k_eigval].cpu().detach().numpy())
                        adj_cond_noise.append(adj_noise[i, :n_nodes[i], :].cpu().detach().numpy())
                        if self.hparams.qm9:
                            #  Add node features as edege "weight"
                            nx_graph = nx.from_numpy_array(edge_features[i, :n_nodes[i], :n_nodes[i]].cpu().detach().numpy())
                            #  Add node features  to nx graph
                        ...
                        out_dict = {**out_dict, f'{test_type}_true_graphs{suffix}': true_graphs, f'{test_type}_fake_graphs{suffix}': fake_graphs, f'{test_type}_true_eigvals{suffix}': true_eigvals, f'{test_type}_fake_eigvals{suffix}': fake_eigvals,
                                                f'{test_type}_spectral_loss{suffix}': spectral_loss, f'{test_type}_normalized_spectral_loss{suffix}': normalized_spectral_loss, 
                                                f'{test_type}_spectral_match_loss{suffix}': spectral_match_loss, f'{test_type}_normalized_spectral_match_loss{suffix}': normalized_spectral_match_loss, f'{test_type}_spectral_fail{suffix}': spectral_fail,
                                                f'{test_type}_forward_pass_time{suffix}': forward_pass_time, f'{test_type}_true_eigvecs{suffix}': true_eigvecs, f'{test_type}_fake_eigvecs{suffix}': fake_eigvecs, f'{test_type}_adj_cond_noise{suffix}': adj_cond_noise}  
            ...
            for dataloader_idx, cond_data_sufix in enumerate(cond_data):
                ...
                if self.hparams.lambda_only:
                    ...
                    for test_type in test_types:
                        ...
                        for measure_type in measure_types:
                            mmd_spectral_filter = spectral_filter_stats(true_eigvecs, true_eigvals, fake_eigvecs, fake_eigvals, compute_emd=(measure_type=='emd'))
                            self.log(f'test_loss_{test_type}{cond_data_sufix}{model_type}/{measure_type}_spectral_filter', mmd_spectral_filter)
                elif self.hparams.lambda_SON_only:
                    test_types = ['all_fake', 'fake_eigvec']
                    for test_type in test_types:
                        self.log(f'test_loss_{test_type}{cond_data_sufix}{model_type}/SON_loss', torch.stack([x[f'{test_type}_SON_loss{model_type}'] for x in outputs_i]).mean())
                    ...
                    for test_type in test_types:
                        ...
                        if self.hparams.no_cond:
                            torch.save([g for x in outputs_i for g in x[f'{test_type}_adj_cond_noise{model_type}']], f"{self.logger.log_dir}/test/{test_type}{cond_data_sufix}{model_type}/adj_cond_noise.pt")
                        ...
                        if self.hparams.qm9:
                            ...
                            for measure_type in measure_types:
                                ...
                                fake_graph_eigvals, fake_graph_eigvecs = compute_list_eigh(fake_graphs)
                                mmd_spectral_filter_graph = spectral_filter_stats(true_graph_eigvecs, true_graph_eigvals, fake_graph_eigvecs, fake_graph_eigvals, compute_emd=(measure_type=='emd'))
                                self.log(f'test_loss_{test_type}{cond_data_sufix}{model_type}/{measure_type}_spectral_filter_graph', mmd_spectral_filter_graph)
...
if __name__ == '__main__':
    ...
    model_params = dict(beta1=args.beta1, beta2=args.beta2, lr_g=args.lr_g, lr_d=args.lr_d, gp_lambda=args.gp_lambda, n_max=args.n_max, 
                        ...
                        ppgn_data_channels_mult=args.ppgn_data_channels_mult, skip_noise_preprocess=args.skip_noise_preprocess, qm9=(args.dataset == 'qm9'))
            ...
            for model_type in model_types:
                if args.dataset == 'qm9':
                    ...
                    callbacks += [
                        ...
                        ModelCheckpoint(
                            monitor=f"validation_loss_{test_type}{model_type}/mmd_spectral_filter_graph",
                            filename=f"best_mmd_spectral_filter_graph_{test_type}{model_type}"+"_{epoch:06d}-{"+f"validation_loss_{test_type}{model_type}/mmd_spectral_filter_graph"+":.4f}",
                            save_top_k=1,
                            mode="min",
                            save_on_train_epoch_end=False,
                            auto_insert_metric_name=False
